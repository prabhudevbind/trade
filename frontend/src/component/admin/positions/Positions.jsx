...
import { Plus, Eye, ArrowUpRight, ArrowDownRight, History, BarChart3, Trophy, TrendingDown } from "lucide-react"
...
import { useGetTradesActiveQuery,useSellPositionMutation } from "@/store/api/contest"
...
export default function PositionsPage() {
  ...
  useEffect(() => {
    ...
    const active = activeTradesData.positions.filter((pos) => pos.net_quantity > 0)
    const closed = activeTradesData.positions.filter((pos) => pos.net_quantity === 0)
  ...
  useEffect(() => {
    ...
    setActivePositions(activeTradesData.positions.filter((pos) => pos.net_quantity > 0))
    setClosedPositions(activeTradesData.positions.filter((pos) => pos.net_quantity === 0))
    setTrades(activeTradesData.trades)
  ...
  const calculateRealizedPnL = (position) => {
    const relatedTrades = trades.filter((trade) => trade.symbol === position.option.symbol)
    let totalBuy = 0
    let totalSell = 0
  ...
  const totalBuyValue = trades.filter((t) => t.action === "buy").reduce((sum, t) => sum + t.price * t.quantity, 0)
  const totalSellValue = trades.filter((t) => t.action === "sell").reduce((sum, t) => sum + t.price * t.quantity, 0)
  ...
  const PositionCard = ({ position, isActive = true }) => {
    ...
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-4 mb-3 shadow-sm">
          ...
          <div className="flex items-center space-x-2">
            <div className="flex-1">
              ...
              <div className="text-xs text-gray-500">
                ...
                {!isActive && <span className="ml-2 text-orange-600">(Closed)</span>}
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-right">
            ...
            <div className={`text-xs ${isProfit ? "text-green-600" : "text-red-600"}`}>
              {isActive ? `${isProfit ? "+" : "-"}${Math.abs(pnlPercentage).toFixed(2)}%` : "Realized"}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-xs text-gray-500 pt-2 border-t border-gray-100">
          ...
          <div>
            <span>Value: ₹{isActive ? (ltp * Math.abs(position.net_quantity)).toFixed(2) : "0.00"}</span>
          </div>
        </div>

        {isActive && position.net_quantity > 0 && (
          <Button
            size="sm"
            variant="outline"
  ...
  const TradeRow = ({ trade }) => {
    ...
    return (
      <TableRow className="hover:bg-gray-50">
        <TableCell>
          <div>
            <div className="font-medium text-gray-900">{trade.option.symbol}</div>
            <div className="text-xs text-gray-500">
        ...
        <TableCell className="text-xs text-gray-500">{new Date(trade.timestamp).toLocaleString()}</TableCell>
      </TableRow>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-2 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <div className="font-semibold text-gray-900">
                ₹{virtualCash.toLocaleString(undefined, { maximumFractionDigits: 2 })}
              </div>
            </div>
            <div className="text-center">
        ...
        <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
          ...
          <div className="flex items-center justify-between text-sm text-gray-600">
            ...
            <span>
              Active: {activeTradesData.summary.activePositions} | Closed: {closedPositions.length}
            </span>
          </div>
        </div>
        ...
        <Tabs defaultValue="positions" className="w-full">
          ...
          <TabsContent value="positions" className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-medium pl-3 text-gray-900">Active Positions ({activePositions.length})</h3>
                {!isMobile && (
                  <Button
                    size="sm"
              ...
              {activePositions.length === 0 ? (
                <div className="bg-white rounded-lg p-8 text-center shadow-sm border border-gray-200">
                  ...
                  <h3 className="font-medium text-gray-900 mb-2">No active positions</h3>
                ...
                <div className="space-y-3">
                  {activePositions.map((position) => (
                    <PositionCard key={position.id} position={position} isActive={true} />
                  ))}
                </div>
              )}
            </div>
          ...
          <TabsContent value="history" className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-medium pl-3 text-gray-900">Closed Positions ({closedPositions.length})</h3>
              </div>
              ...
              {closedPositions.length === 0 ? (
                <div className="bg-white rounded-lg p-8 text-center shadow-sm border border-gray-200">
                  ...
                  <h3 className="font-medium text-gray-900 mb-2">No closed positions</h3>
                  <p className="text-sm text-gray-600">Your completed trades will appear here</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {closedPositions.map((position) => (
                    <PositionCard key={position.id} position={position} isActive={false} />
                  ))}
                </div>
              )}
            </div>
          ...
          <TabsContent value="trades" className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-medium pl-3 text-gray-900">All Trades ({trades.length})</h3>
              </div>
      ...
      {/* Sell Modal */}
      <Dialog open={sellModal.open} onOpenChange={(open) => setSellModal({ open, position: null })}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button
                onClick={handleSell}
                disabled={sellLoading || sellQty < 1 || sellQty > (sellModal.position?.net_quantity || 0)}
